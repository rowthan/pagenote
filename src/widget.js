    ...
    <div style={{position:"absolute",top:step.y+"px",left:step.x+"px"}}>
      ...
      </aside>
      {
        ...
        <div className={style.box}>
          <span contentEditable={step.writing===true}
          // 节流处理 oninput
            oninput={(e)=>{const value = e.target.innerText; step.modify = value;actions.refershState()}}
            style={{width:"100%",display:"block",border:`${step.writing?1:0}px solid`}}>
            {step.tip}
          </span>
          {(step.modify!=undefined && step.modify!=step.tip ) && "有改动,请保存"}
          <span className={style.edit} onclick={()=>{step.writing=true}}>
          ...
          </span>
          {
            step.writing &&
            <div>
              <a onclick={()=>easyshare.remove(index)} title="删除">
                <svg viewBox="0 0 1024 1024" version="1.1" width="20" height="20">
