        ...
        <Form {...form}>
...

          {
              disableList.map((item, index) => {
                  return (
                      <div className={'flex items-center justify-between text-sm p-2 rounded hover:bg-card border-b'} key={index}>
                          <span className={'max-w-4/5 overflow-hidden'}>{item}</span>
                          <button onClick={()=>{removeRule(item)}}>
                            <AiOutlineDelete />
                          </button>
                      </div>
                  )
