...
const checkRelease = function (limitSetting:FrequencyOption,taskHistory:TaskMeta[]):boolean {
    const currentTime = new Date().getTime();

    const {duration,limit,parallel,parallelDuration=PARALLEL_DURATION} = limitSetting;
...
class Schedule{
    ...
    constructor(frequency:FrequencyOption) {
        ...
        this.retryTimeout = frequency.duration / frequency.limit
    ...
    // 添加方法
    addFun<T>(fun:Task,options:TaskOption) {
    ...
    runFun ():boolean {
        ...
        this.status = releaseResult ? SCHEDULE_STATUS.running : SCHEDULE_STATUS.pause;
        if(releaseResult){
            ...
            try{
                ...
                console.warn('传入方法执行错误，不会重试。请在外部 catch 后检查是否需要重新加入任务列表',currentTask)
                throw Error(e)
            }
